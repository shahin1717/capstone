...
\author{Shahin Alakparov \and Murad Asgarov \and Gurban Burjuzada \and Davud Gurbanov}
\date{}
...
\begin{abstract}
This project is studying the question of when a single hidden layer Neural Network is better than linear softmax regression model and when additional complexity is unnecessary. In this work we have implemented both models from scratch using NumPy across 3 different tasks: linearly separable Gaussian task, nonlinear moons task and digits dataset. Results of this experiment suggests that linear models are enough when the data is close to being linearly separated, and hidden layer NN gives an advantage when the boundaries are non-linear. On the digits dataset, we show that NN performs moderately as in accuracy but far more better in generalization. We also
