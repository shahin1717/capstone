...
\begin{figure}[h]
    ...
    \caption{Decision boundaries on the linear Gaussian task. The linear model (left) is sufficient, while the neural network (right) introduces unnecessary nonlinear artifacts.}
...
\begin{figure}[h]
    ...
    \caption{Decision boundaries on the Moons dataset. Softmax regression (left) fails to capture the geometry, whereas the neural network (right) perfectly separates the classes.}
...
\begin{figure}[h]
    ...
    \caption{Training dynamics on the Digits benchmark using standard SGD. The neural network exhibits a long flat plateau before escaping the saddle point.}
...
\begin{figure}[h]
    ...
    \caption{Capacity ablation on the Moons task comparing hidden widths of 2, 8, and 32. A width of 8 provides the optimal balance without underfitting or overfitting.}
...
\begin{figure}[h]
    ...
    \caption{Optimizer study on the Digits benchmark ($h=32$). Adam converges almost instantly, while SGD plateaus and Momentum fails entirely with the default learning rate.}
...
\begin{figure}[h]
    ...
    \caption{Failure case analysis for the neural network with $h=2$ on the Moons task, demonstrating a fundamental lack of geometric capacity to enclose the crescent shapes.}
